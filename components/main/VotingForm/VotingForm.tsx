...
export const VotingForm: FC<VotingFormProps> = memo(({ options, sendMessage }) => {
  const debounceCall = useDebouncedCallback((value) => sendMessage({ type: 'vote', optionId: value }), FORM_DEBOUNCE_TIME, { leading: true })

  const handleSubmit = useCallback((event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    debounceCall((event.currentTarget.elements.namedItem('vote') as HTMLInputElement).value as OptionId)
  }, [])
