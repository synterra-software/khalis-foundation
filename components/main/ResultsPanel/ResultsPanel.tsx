...
export const ResultsPanel: FC<ResultsPanelProps> = memo(({ status, results }) => {
  if (status === 'Error') {
    return (
      <div>
        <h2>Error</h2>
  ...
  const totalCount = Object.values(results).reduce((acc, curr) => acc + curr, 0)
          ...
          return (
            <div key={result[0]}>
              ...
              <div className='bg-blue-50 rounded-3xl overflow-hidden'>
                <div className='bg-blue-500 h-2' style={{ width: result[1] > 0 ? `${percents}%` : '0%' }} />
              </div>
            </div>
          )
        })}
